...
# # 
# # write(UTR3_fasta,"/home/ben/Analysis/RF_human/Library_V2_Aug2021/3UTR_ensembl_r104_formatted.txt")
# 
...
# # UTR3_fasta$tx.id <- mapply(strsplit(as.character(UTR3_fasta$V1),"\\>"),FUN=function(x){(as.character(x)[1])})
# 
...
ATTRACT <- read.delim("/home/ben/Analysis/ATtRACT/ATtRACT_db.txt")
...
dim(ATTRACT[!duplicated(ATTRACT$Gene_name),])
...
# 
# UTR3_fasta_export$sequence <- NULL
# 
# colnames(UTR3_fasta_export) <- paste0(colnames(UTR3_fasta_export),"_UTR3")
# # registerDoMC(4)
...
# print(dim(UTR3_fasta_export)) 
# 
# write.table(UTR3_fasta_export,"/home/ben/Analysis/RF_human/Library_V2_Aug2021/3UTR_library_v2_RBP_GC_length.csv",sep = ";", dec = ",",row.names = F)


UTR3_fasta_export <- read.delim("/home/ben/Analysis/RF_human/Library_V2_Aug2021/3UTR_library_v2_RBP_GC_length.csv",sep = ";", dec = ",")
dim(UTR3_fasta_export) #  2269 -3  non RBP motifs = 
...
# CDS_fasta$tx.id <- mapply(strsplit(as.character(CDS_fasta$V1),"\\>"),FUN=function(x){(as.character(x)[1])})
...
#write.table(CDS_database_export,"RBP_motifs_in_human_transcriptome_3UTR_hg38_from_ATtRACT_db.csv", sep=";",dec=",", row.names =F, quote = F)
...
# CDS_fasta$CDS_length <- NULL
# CDS_fasta_temp <- merge(CDS_fasta,CDS_fasta_export,by.x="tx.id",by.y="tx.id_CDS")
# CDS_fasta <- CDS_fasta_temp[,1:2272]
...
CDS_fasta_codon_count <- Biostrings::DNAStringSet(CDS_fasta_codon$seq,use.names = T)
...
table(mapply(strsplit(as.character(CDS_fasta$sequence),""),FUN=function(x){(as.character(x)[1])}))

CDS_fasta$sequence <- gsub("N","A",CDS_fasta$sequence)

CDS_fasta$translation <- mapply(strsplit(as.character(CDS_fasta$sequence),","),FUN=function(x){as.character(translate(Biostrings::DNAString(x)))})
...
CDS_fasta$STP <- NULL
CDS_fasta[2339:2358] <- (CDS_fasta[2339:2358]/CDS_fasta$AA_length)*100
...
#colnames(CDS_fasta_export) <- gsub("_CDS","",colnames(CDS_fasta_export))
colnames(CDS_fasta_export) <- paste0(colnames(CDS_fasta_export),"_CDS")
# registerDoMC(4)
# Gene_wise_3UTR_list <- ddply(Gene_wise_3UTR_list,"external_gene_name", numcolwise(mean), .parallel = T, .progress = T)
#Gene_wise_3UTR_list$ARE_counts <- ceiling(Gene_wise_3UTR_list$ARE_counts)
print(dim(CDS_fasta_export)) 

write.table(CDS_fasta_export,"/home/ben/Analysis/RF_human/Library_V2_Aug2021/CDS_library_v2_RBP_GC_length.csv",sep = ";", dec = ",",row.names = F)

CDS_fasta_export <- read.delim("/home/ben/Analysis/RF_human/Library_V2_Aug2021/CDS_library_v2_RBP_GC_length.csv",sep = ";", dec = ",")
dim(CDS_fasta_export) # 2358 -4 -64 -20 =2270
...
# # UTR5_fasta$tx.id <- mapply(strsplit(as.character(UTR5_fasta$V1),"\\>"),FUN=function(x){(as.character(x)[1])})
# 
...
# 
# #write.table(UTR5_database_export,"RBP_motifs_in_human_transcriptome_3UTR_hg38_from_ATtRACT_db.csv", sep=";",dec=",", row.names =F, quote = F)
# 
...
# # Gene_wise_3UTR_list <- ddply(Gene_wise_3UTR_list,"external_gene_name", numcolwise(mean), .parallel = T, .progress = T)
# #Gene_wise_3UTR_list$ARE_counts <- ceiling(Gene_wise_3UTR_list$ARE_counts)
# print(dim(UTR5_fasta_export)) 
# 
# write.table(UTR5_fasta_export,"/home/ben/Analysis/RF_human/Library_V2_Aug2021/5UTR_library_v2_RBP_GC_length.csv",sep = ";", dec = ",",row.names = F)
# 

UTR5_fasta_export <- read.delim("/home/ben/Analysis/RF_human/Library_V2_Aug2021/5UTR_library_v2_RBP_GC_length.csv",sep = ";", dec = ",")
dim(UTR5_fasta_export) # 2250 - 3 non RBP motifs = 
...
m6A <- read.delim("/home/ben/Analysis/RF_human/Library_V2_Aug2021/RMVar_Human_basic_info_m6A.txt",sep="\t")
dim(m6A)
head(m6A)
...
### m5C ###
m5C <- read.delim("/home/ben/Analysis/RF_human/Library_V2_Aug2021/RMVar_Human_basic_info_m5C.txt",sep="\t")
...
### AtoI ###
AtoI <- read.delim("/home/ben/Analysis/RF_human/Library_V2_Aug2021/RMVar_Human_basic_info_A-to-I.txt",sep="\t")
...
### m1A ###
m1A <- read.delim("/home/ben/Analysis/RF_human/Library_V2_Aug2021/RMVar_Human_basic_info_m1A.txt",sep="\t")
...
### m7G ###
m7G <- read.delim("/home/ben/Analysis/RF_human/Library_V2_Aug2021/RMVar_Human_basic_info_m7G.txt",sep="\t")
...
UTR5_fasta <- read.delim("/home/ben/Analysis/RF_human/Library_V2_Aug2021/5UTR_library_v2_RBP_GC_length.csv",sep=";", dec=",")
CDS_fasta <- read.delim("/home/ben/Analysis/RF_human/Library_V2_Aug2021/CDS_library_v2_RBP_GC_length.csv",sep=";", dec=",")
...
dim(Library_V2)
table(duplicated(Library_V2$tx.id))
